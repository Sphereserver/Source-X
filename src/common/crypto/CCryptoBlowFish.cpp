...
static constexpr dword s_kdwInitData[18+1024] =
{
    // @0106248 in v2.0.3
...
static constexpr byte s_kKeyTable[CRYPT_GAMEKEY_COUNT][CRYPT_GAMEKEY_LENGTH] =
{
    // @ 0f1420 in v2.0.3
...
static constexpr byte sm_kSeedTable[2][CRYPT_GAMEKEY_COUNT][2][CRYPT_GAMESEED_LENGTH] =
{
...
bool CCrypto::sm_fBFishTablesReady = false; // static
...
void CCrypto::InitTables() // static
{
	...
	for (int i=0; i<CRYPT_GAMEKEY_COUNT; ++i)
	{
		...
		{
			PrepareKey( tmpKey, i );
			s_dwCodingData[i][j+0] = tmpKey.u_iKey[1];
			s_dwCodingData[i][j+1] = tmpKey.u_iKey[0];
		}
	}
...
void CCrypto::InitBlowFish()
{
	ADDTOCALLSTACK("CCrypto::InitBlowFish");
	if ( !sm_fBFishTablesReady )
		InitTables();
